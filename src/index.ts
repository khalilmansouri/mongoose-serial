...
export const extractCounter = (options: Options, serial: string): string => {
  let { separator, initCounter, digits = 10 , startFrom } = options;
  let counter: string;
...
export const plugin = (schema: Schema, options: Options) => {
  ...
  schema.pre("save", async function (next) {
    ...
    let lastDoc;
    try {
      lastDoc = await (<any>this).constructor
        .findOne({})
        .sort({ [`${field}`]: -1 });
    } catch (error) {
      throw error;
    ...
    switch (initCounter) {
      ...
      case InitCounter.MONTHLY:
        dating = [
          ...
          addZeros(new Date().getMonth() + 1, 2),
        ].join(separator);
        break;
      case InitCounter.DAILY:
        dating = [
